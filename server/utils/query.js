...
//并且将数据库文件执行，执行完毕后lwc_blog数据库就有对应的表和数据了
const init = mysql.createConnection(db)
init.connect()
init.query('CREATE DATABASE lwc_blog',err=>{
    Object.assign(db,dbName)
    //第二次连接数据库，这时候，数据库lwc_blog已经创建成功了，这时候，直接连接lwc_blog数据库
    //然后执行sql文件夹下的lwc_blog.sql文件，对应的表和测试数据就已经存在数据库里面了
    pool = mysql.createPool(db)
    if(err){
        console.log('lwc_blog database created already')
    }else{
        console.log('create lwc_blog Database')
